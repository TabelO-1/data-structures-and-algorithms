...
Write a function named transformToLis that, given an object, returns an array of the key value pairs as 
html list items.
...
const count = (target, input) => {
  let num = 0;
  input.filter(arr => arr.map(item => item === target ? num++ : false));
  return num;
};
...
const totalSum = (input) => {
  let num = 0;
  input.map(numArr => numArr.map(item => num+=item));
  return num;
};
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  // return .map(item => Math.pow(item, 2));
};
//variable % 2 === 0 from stack overflow
//Math.pow(7, 3) from mdn web docs
/* ------------------------------------------------------------------------------------------------
CHALLENGE 5
...
uses any combination of filter, map and reduce to return the name of the character who is the s
hortest in height.
------------------------------------------------------------------------------------------------ */

let findShortest = (data) => {
  return data.reduce((short, next) => {
    return Number(short.height) < Number(next.height) ? short : next;
  }).name
};
//code from https://www.codermen.com/find-longest-shortest-and-random-string-in-array-in-javascript/
